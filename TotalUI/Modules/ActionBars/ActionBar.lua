...
function ActionBar:CreateButtons()
    ...
    for i = 1, self.db.buttons do
        local button = self:CreateButton(i, LAB)
        if button then
...
function ActionBar:CreateButton(buttonID, LAB)
    ...
    local button = LAB:CreateButton(actionID, buttonName, self.frame, config)
    ...
    LAB:SetButtonSize(button, self.db.buttonSize, self.db.buttonHeight or self.db.buttonSize)
